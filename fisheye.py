...
def fish(img, cx, cy, distortion_coefficient):
    """
    :type img: numpy.ndarray
    :param cx: x-coordinate Center of the lens.
    :param cy: y-coordinate Center of the lens.
    ...
    for x in range(len(dstimg)):
        for y in range(len(dstimg[x])):
            ...
            ncx, ncy = float((2*cx - w)/w), float((2*cy - h)/h)
            # get xn and yn distance from normalized center
            rd = sqrt((xnd-ncx)**2 + (ynd-ncy)**2)
